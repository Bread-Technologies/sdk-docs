...
<Note>
Iterative baking builds on [single-prompt](/sdk/guides/single-prompt-bake) and [multi-target](/sdk/guides/multi-target-baking) baking. Complete those first.
</Note>
...
// Baked model path, for example: "johndoe/yoda_repo/bake1/35"
const previousBakedModel = '[username]/[repo]/[bake]/[checkpoint_num]';
```
...
```typescript TypeScript
await client.repo.create({ repo_name: 'yoda_v2' });

// Teacher: More specific, refined guidance
await client.prompts.create('yoda_v2', {
  prompt_name: 'yoda_teacher_prompt_2',
  messages: [{
    role: 'system',
...
```typescript TypeScript
// Add stim prompts to ellicit more Yoda-like behavior
await client.targets.create('yoda_v2', {
  target_name: 'yoda_target_2',
  template: 'default',
  overrides: {
    generators: [{
    ...
    model_name: previousBakedModel,  // Use Yoda v1 as base
    teacher_prompt: 'yoda_teacher_prompt_2',
    student_prompt: 'empty_student',
  },
});
...
// Generate training data
await client.targets.stim.run('yoda_target_2', {
  repo_name: 'yoda_v2',
});
await client.targets.rollout.run('yoda_target_2', {
  repo_name: 'yoda_v2',
});
```
...
```typescript TypeScript
await client.bakes.create('yoda_v2', {
  bake_name: 'yoda_bake_v2',
  template: 'default',
  overrides: {
    datasets: [
    ...
    model: {
      type: 'bake',
      parent_model_name: previousBakedModel,  // Start from Yoda v1
      baked_adapter_config: {
        r: 8,
        lora_alpha: 16,
      },
...
await client.bakes.run('yoda_bake_v2', {
  repo_name: 'yoda_v2',
});
```
...
// Iteration 3: Add mysticism
await client.repo.create({ repo_name: 'yoda_v3' });
// [Create refinement target focused on Force wisdom]
// Use yoda_v2 as base model
// Result: yoda_v3 (brief + Yoda + mystical)
```
