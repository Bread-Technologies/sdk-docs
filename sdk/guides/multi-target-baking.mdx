...
<Note>
Completed [your first bake](/sdk/guides/single-prompt-bake)? Multi-target baking builds on that foundation.
</Note>
...
// Shared student prompt (empty for always-on)
await client.prompts.create('apple_support_agent', {
  prompt_name: 'empty_student_prompt',
  messages: [{ role: 'system', content: '' }],
});
```
...
await client.prompts.create('apple_support_agent', {
  ...
  messages: [{
    role: 'system',
    content: `You are an Apple iPhone support expert. You know:
...
await client.targets.create('apple_support_agent', {
  target_name: 'iphone_support',
  template: 'default',
  overrides: {
    generators: [
...
// Generate stim data
await client.targets.stim.run('iphone_support', {
  repo_name: 'apple_support_agent',
});
// Once stim complete, generate rollout data
await client.targets.rollout.run('iphone_support', {
  repo_name: 'apple_support_agent',
});
```
...
await client.prompts.create('apple_support_agent', {
  ...
  messages: [{
    role: 'system',
    content: `You are an Apple Mac support expert. You know:
...
await client.targets.stim.run('mac_support', {
  repo_name: 'apple_support_agent',
});
await client.targets.rollout.run('mac_support', {
  repo_name: 'apple_support_agent',
});
```
...
await client.prompts.create('apple_support_agent', {
  ...
  messages: [{
    role: 'system',
    content: `You are an Apple AirPods support expert. You know:
...
await client.targets.stim.run('airpods_support', {
  repo_name: 'apple_support_agent',
});
await client.targets.rollout.run('airpods_support', {
  repo_name: 'apple_support_agent',
});
```
...
```typescript TypeScript
await client.bakes.create('apple_support_agent', {
  bake_name: 'apple_support_agent_v1',
  template: 'default',
  overrides: {
    datasets: [
...
```typescript TypeScript
await client.bakes.run('apple_support_agent_v1', {
  repo_name: 'apple_support_agent',
});
```
...
<AccordionGroup>
  <Accordion title="Generate Diverse Training Data" icon="sparkles">
    Use [good generators](/sdk/configuration/generators) for each target to create diverse, high-quality training examples.
  </Accordion>

  <Accordion title="Monitor Target Balance" icon="chart-line">
    Use [Weights & Biases integration](/sdk/configuration/bake-config#wandb-integration) to track how each target influences training.
  </Accordion>
