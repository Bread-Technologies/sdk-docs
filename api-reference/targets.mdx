...
<ParamField path="teacher_prompt" type="string">
  Teacher prompt name - prompt that defines the target behavior being baked into the model. Also accepts deprecated `u` field.
</ParamField>

<ParamField path="student_prompt" type="string">
  Student prompt name - simplified prompt given at inference. After baking, this triggers the teacher behavior. Can be empty (`""`) for always-on behavior. Also accepts deprecated `v` field.
</ParamField>
