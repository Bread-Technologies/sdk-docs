...
- Python: Not yet available. Use REST API directly.
...
<Note>
**SDK Usage**: 
- TypeScript: Not yet available. Use REST API directly.
- Python: Not yet available. Use REST API directly.
...
**Response Fields**:
- `bakes`: `Array<BakeNode>` - List of bakes with lineage information
  - `name`: `string` - Bake name
  - `status`: `string` - Bake status: 'not_started', 'running', 'complete', or 'failed'
  - `model_name`: `Array<string> | null` - List of model checkpoints (when complete)
  - `parent`: `string | null` - Parent bake name (if any)
  - `children`: `Array<string>` - List of child bake names
