...
<AccordionGroup>
  ...
  <Accordion title="The Math: KL Divergence Minimization" icon="function">
    ...
    - **P<sub>θ</sub>(·|u)**: Original model with prompt `u`
    - **P<sub>θ_u</sub>(·)**: Baked model without prompt
    
    We update weights `θ` → `θ_u` to minimize KL divergence between these distributions. The result: the baked model behaves identically to the prompted model, but without needing the prompt.
